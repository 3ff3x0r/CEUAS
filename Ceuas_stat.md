...
```
    ...
    /home/leon/anaconda3/lib/python3.10/site-packages/seaborn/categorical.py:1273: FutureWarning: DataFrameGroupBy.apply operated on the grouping columns. This behavior is deprecated, and in a future version of pandas the grouping columns will be excluded from the operation. Either pass `include_groups=False` to exclude the groupings or explicitly select the grouping columns after groupby to silence this warning.
      .apply(aggregator, agg_var)
    /tmp/ipykernel_29209/3732651780.py:81: UserWarning: set_ticklabels() should only be used with a fixed number of ticks, i.e. after set_ticks() or using a FixedLocator.
      ax.set_yticklabels(wrapped_labels)
...
def main():
    ...
    if members_df is not None and not members_df.empty:
        ...
        plt.ylabel('Number of Respondents', fontsize=12)
        plt.title('Distribution of CEUA Size (Number of Members)', fontsize=14, fontweight='bold')
...
def main():
    ...
    if licenses_df is not None and not licenses_df.empty:
        ...
        plt.ylabel('Number of Respondents', fontsize=12)
        plt.title('Distribution of Refused Licenses Reported by Members', fontsize=14, fontweight='bold')
...
def main():
    ...
    if adhoc_df is not None and not adhoc_df.empty:
        ...
        plt.ylabel('Number of Respondents', fontsize=12)
        plt.title('Distribution of Ad-hoc Consultant Use Reported by Members', fontsize=14, fontweight='bold')
        # Rotate labels to prevent overlap.
        plt.xticks(rotation=45, ha='right')
...
def main():
    ...
    if courses_df is not None and not courses_df.empty:
        ...
        plt.ylabel('Number of Respondents', fontsize=12)
        plt.title('Frequency of CEUA Educational Activities Reported by Members', fontsize=14, fontweight='bold')
...
def main():
    ...
    if meetings_df is not None and not meetings_df.empty:
        ...
        plt.ylabel('Number of Respondents', fontsize=12)
        plt.title('Frequency of Supplementary CEUA Meetings Reported by Members', fontsize=14, fontweight='bold')
...
def main():
    ...
    if decision_df is not None and not decision_df.empty:
        ...
        plt.ylabel('Number of Respondents', fontsize=12)
        plt.title('Distribution of CEUA Decision-Making Methods Reported by Members', fontsize=14, fontweight='bold')
...
def get_active_respondent_meeting_periodicity(db_name):
    ...
    try:
        conn = sqlite3.connect(db_name)
        # SQL query to get the meeting periodicity data for active respondents.
        query = """
        SELECT
...
def main():
    """
    Main function to perform and display the univariate analysis of 'Periodicidade_das_reunioes'.
    """
    ...
    if periodicity_df is not None and not periodicity_df.empty:
        ...
        # Clean up potential 'Other:' prefixes if they exist
        periodicity_df['MeetingPeriodicity'] = periodicity_df['MeetingPeriodicity'].str.replace('Other:', 'Other', regex=False).str.strip()
        ...
        periodicity_df['MeetingPeriodicity'] = periodicity_df['MeetingPeriodicity'].replace(translation_map)

        # --- Aggregation Step ---
        # Identify the top 6 most common categories.
        value_counts = periodicity_df['MeetingPeriodicity'].value_counts()
        if len(value_counts) > 6:
            top_6_categories = value_counts.nlargest(6).index.tolist()
            
            # Replace all other categories with 'Other'
            periodicity_df.loc[~periodicity_df['MeetingPeriodicity'].isin(top_6_categories), 'MeetingPeriodicity'] = 'Other'
            print("Aggregated less frequent responses into 'Other' category.")

        # --- 1. Descriptive Statistics (on aggregated data) ---
        print("\n--- Descriptive Statistics for Meeting Periodicity (Aggregated) ---")
        print(periodicity_df['MeetingPeriodicity'].describe())
        
        print("\nFrequency Count (Aggregated):")
        print(periodicity_df['MeetingPeriodicity'].value_counts())


        # --- 2. Plotting the Data ---
        print("\n--- Generating Meeting Periodicity Distribution Plot (Aggregated) ---")
        
        # Define the logical order for the categories, ensuring 'Other' is last.
        base_order = [
            'Once a week',
            'Every fifteen days',
            'Once a month',
        ...
        # Filter the base order to include only categories present in the data
        final_categories = periodicity_df['MeetingPeriodicity'].unique()
        category_order = [cat for cat in base_order if cat in final_categories]
        
        # Add 'Other' to the end if it exists
        if 'Other' in final_categories:
            category_order.append('Other')
        ...
        plt.ylabel('Number of Respondents', fontsize=12)
        plt.title('Distribution of CEUA Meeting Periodicity Reported by Members', fontsize=14, fontweight='bold')
        plt.xticks(rotation=45, ha='right')
        
        total_respondents = len(periodicity_df['MeetingPeriodicity'].dropna())
        ...
        # Adjust top margin to ensure labels fit.
        max_height = periodicity_df['MeetingPeriodicity'].value_counts().max()
        ax.set_ylim(0, max_height * 1.15)
...
```
    ...
    Aggregated less frequent responses into 'Other' category.
    ...
    top       Once a month
    freq               223
    Name: MeetingPeriodicity, dtype: object
    
    Frequency Count (Aggregated):
    MeetingPeriodicity
    Once a month             223
    Every fifteen days        38
    Once every two months     37
    Other                     30
...
def main():
    ...
    if spa_df is not None and not spa_df.empty:
        ...
        plt.title('Does SPA Representative Handle Protocol Reporting?', fontsize=14, fontweight='bold')
...
def main():
    ...
    if justification_df is not None and not justification_df.empty:
        ...
        translation_map = {
            ...
            'Falta de Competências, Conhecimentos Técnicos/Científicos (incluindo em BEA), treinamento, formação superior ou capacidades (incluindo argumentativa). Falta de imparcialidade/Antiético;SPA é passional/pode haver extremismo e conflito; Pessoas de dificil convivência': "SPA lacks competence",
            'Falta de engajamento, participação, comunicação/contato, presença, constância, abertura, diálogo/não há interesse por participar. Falta de engajamento da SPA; dificuldade em encontrar representantes': "SPA lacks engagement",
            'Desde que: tenha formação superior ou conhecimento técnico/científico, ou receba ajuda/orientação/contribuição de outro membro/conheça as normativas do CONCEA/seja qualificado/tenha disponibilidade.': "Conditional on SPA training",
            ...
            'Contribuição positiva: Gera Aprendizado para a CEUA ou para o membro SPA ou aumenta engajamento do SPA, visão externa e Diversidade de opiniões ajudam, melhor distribuição de tarefas. Função educativa (para SPA ou a CEUA ). Positivo para a defesa ou proteção dos animais, positivo para o BEA.Representam os interesses dos animais; protegem decisão ética/correta.': "SPA contributes positively",
            ...
            'Não é função do SPA. Regra do CONCEA, Papel exclusivo, para membros institucionais, internos ou com certos papeis (médico veterinário, zootecnista ou biólogo). Papel exclusivo de certos membros; Riscos (confidencialidade). Desnecessário; não há benefício; não acrescenta; os outros membros são suficientes; indiferente': "Outside of SPA's scope",
            'Falta de imparcialidade/Antiético;SPA é passional/pode haver extremismo e conflito; Pessoas de dificil convivência': "Risk of partiality/extremism"
        }
        justification_df['Justification'] = justification_df['Justification'].replace(translation_map)
        
        # Wrap text for better display on the y-axis
        ...
        # Order categories by frequency for clarity
        category_order = justification_df['Justification'].value_counts().index
        ...
        plt.title("Justifications for SPA Representative's Role in Protocol Reporting", fontsize=14, fontweight='bold')
...
```
    ...
    count                      369
    unique                       8
    top       SPA lacks competence
    freq                        89
    Name: Justification, dtype: object
    
    Frequency Count:
    Justification
    SPA lacks competence             89
    Conditional on SPA training      73
    Based on equality/isonomy        62
    No opinion / Not classifiable    49
    SPA contributes positively       36
    SPA is competent for the role    33
    Outside of SPA's scope           16
    SPA lacks engagement             11
    Name: count, dtype: int64
...
```
    ...
    /tmp/ipykernel_29209/397042003.py:149: FutureWarning: The behavior of array concatenation with empty entries is deprecated. In a future version, this will no longer exclude empty items when determining the result dtype. To retain the old behavior, exclude the empty entries before the concat operation.
      df[col] = numeric_part.combine_first(text_part)
...
```
    ...
    /tmp/ipykernel_29209/3014332350.py:65: FutureWarning: 
    
    Passing `palette` without assigning `hue` is deprecated and will be removed in v0.14.0. Assign the `x` variable to `hue` and set `legend=False` for the same effect.
    
      ax = sns.violinplot(x='IsVegan', y='SufferingScale', data=df,
    /tmp/ipykernel_29209/3014332350.py:70: UserWarning: set_ticklabels() should only be used with a fixed number of ticks, i.e. after set_ticks() or using a FixedLocator.
      ax.set_xticklabels(['Non-Vegan/Vegetarian', 'Vegan/Vegetarian'])
